/*
    ...
    Modified by Phuong's addition of 2 JButtons
 ...
 public class LandscapeDisplay
{
    ...
    public LandscapeDisplay(Board scape, int scale)
    {
        ...
        this.win.add(reset, BorderLayout.SOUTH);
        this.win.add(solve, BorderLayout.NORTH);
    ...
    //a method that add ActionListener to the JButtons
    public void addSudoku(Sudoku S) {
        ClickActionListener makeIt = new ClickActionListener();
        makeIt.resetSudoku(S);
        reset.addActionListener( makeIt );

        SolveActionListener solveIt = new SolveActionListener();
        solveIt.solveSudoku(S);
...
// private class implements ActionListener that evoke the method in Sudoku class
    private class ClickActionListener implements ActionListener {private Sudoku S; public void resetSudoku(Sudoku S) {
        this.S = S;

    }
        public void actionPerformed(ActionEvent e) {
            System.out.println("Click button activated");
            S.reset();
        }
    }

// private class implements ActionListener that evoke the method in Sudoku class
    private class SolveActionListener implements ActionListener {private Sudoku X; public void solveSudoku(Sudoku X) {
        this.X = X;
    } 
        public void actionPerformed(ActionEvent e) {
            X.solve(100);
